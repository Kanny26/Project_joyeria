...
public class PDFGenerator {
    ...
    public static byte[] generarFacturaPDF(Venta venta) throws Exception {
        ...
        for (String h : headers) {
            PdfPCell headerCell = new PdfPCell(new Phrase(h, FontFactory.getFont(FontFactory.HELVETICA_BOLD, 9, BaseColor.WHITE)));
        ...
        if ("anticipo".equals(venta.getModalidad()) && venta.getMontoAnticipo() != null) {
            ...
            if (venta.getSaldoPendiente() != null && venta.getSaldoPendiente().compareTo(BigDecimal.ZERO) > 0) {
                anticipo.addCell(new PdfPCell(new Phrase("Saldo Pendiente:", FontFactory.getFont(FontFactory.HELVETICA_BOLD, 10, COLOR_PENDIENTE))));
                anticipo.addCell(new PdfPCell(new Phrase(formatoMoneda(venta.getSaldoPendiente()), FontFactory.getFont(FontFactory.HELVETICA_BOLD, 10, COLOR_PENDIENTE))));
            }
