...
<main class="product-page">
    <form action="<%= request.getContextPath() %>/ProductoServlet" method="post" enctype="multipart/form-data" class="full-width">
        ...
        <section class="product-card">
            <div class="product-image">
                <div class="product-image__circle">
                    <img id="preview" src="<%= request.getContextPath() %>/imagen-producto/<%= producto.getProductoId() %>" alt="Vista previa">
                </div>
                <div class="custom-file-upload">
                    <label for="imagenInput" class="btn-upload"><i class="fa-solid fa-cloud-arrow-up"></i> Cambiar Imagen</label>
                    <input type="file" name="imagen" id="imagenInput" accept="image/*" onchange="handleImageChange(this)">
            ...
            <div class="product-details">
                <div class="product-row">
                    ...
                    <input class="product-value" type="text" name="nombre" value="<%= producto.getNombre() %>" required>
                </div>
                <div class="product-row">
                    <label class="product-label">Precio Costo</label>
                    <input class="product-value" type="number" step="0.01" name="precioUnitario" value="<%= producto.getPrecioUnitario() %>" required>
                </div>
                <div class="product-row">
                    <label class="product-label">Precio Venta</label>
                    <input class="product-value" type="number" step="0.01" name="precioVenta" value="<%= producto.getPrecioVenta() %>" required>
                </div>
                <div class="product-row">
                    <label class="product-label">Material</label>
                    <select class="product-value" name="materialId" required>
                        <% for (Material m : materiales) { %>
                            <option value="<%= m.getMaterialId() %>" <%= producto.getMaterial().getMaterialId() == m.getMaterialId() ? "selected" : "" %>><%= m.getNombre() %></option>
                        <% } %>
                    </select>
                </div>
                <div class="product-row">
